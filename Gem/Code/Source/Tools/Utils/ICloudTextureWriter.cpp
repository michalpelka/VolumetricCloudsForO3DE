...
{
    ICloudTextureWriter::ICloudTextureWriter(
        uint16_t mipLevels, AZ::RHI::Format pixelFormat, const AZ::IO::Path& outputDir, const AZStd::string& stemPrefix)
        : m_mipLevels(mipLevels)
        , m_pixelFormat(pixelFormat)
        , m_outputDir(outputDir)
        , m_stemPrefix(stemPrefix)
    {
        m_mipLevelsDataList.reserve(mipLevels);
        m_mipLevelsDataList.resize(mipLevels);
    }

    bool ICloudTextureWriter::SetDataBufferForMipLevel(
        AZStd::shared_ptr<AZStd::vector<uint8_t>> dataBuffer, uint16_t mipLevel, AZ::RHI::Size mipSize)
    {
        ...
        AZ_Assert(
            ...
            mipLevel,
            m_mipLevelsDataList.size());
        AZ_Warning(
            GetLogName(),
            !m_mipLevelsDataList[mipLevel].m_dataBuffer,
            "mipLevel %hu already had a data buffer and will be replaced.\n",
            mipLevel);
